// Original, browser-synthesized ambient music. No recording, network or autoplay.
...
let context, master, timer, nextNote = 0, step = 0, enabled = false, busy = false;
...
function updateUI() {
  ...
  button.textContent = enabled ? '♫ 음악 끄기' : '♫ 음악 켜기';
...
button.addEventListener('click', async () => {
  ...
  try {
    if (enabled) await stop();
      ...
      if (context.state !== 'running') throw new Error('오디오 재생 허용 필요');
      ...
      step = 0;
      nextNote = context.currentTime + 0.06;
      enabled = true;
      schedule();
...
document.addEventListener('visibilitychange', () => { if (document.hidden && enabled) void stop(); });
window.addEventListener('pagehide', () => { if (enabled) void stop(); });
window.musicDiagnostics = () => ({enabled, contextState: context?.state || 'none', timerActive: timer !== undefined, volume: Number(slider.value)});
